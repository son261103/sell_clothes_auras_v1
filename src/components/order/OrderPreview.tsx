...
const OrderPreview: React.FC<OrderPreviewProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-primary/10 dark:border-primary/20 p-6 shadow-sm">
            ...
            <div className="mb-4">
                ...
                {shippingMethod ? (
                    <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <p className="text-textDark dark:text-textLight">{shippingMethod.name} - {shippingMethod.estimatedDeliveryTime}</p>
                    </div>
                ) : (
            ...
            <div className="mb-4">
                <h3 className="text-lg font-semibold text-textDark dark:text-textLight mb-2">Sản phẩm ({orderItems.length})</h3>
                <div className="max-h-48 overflow-y-auto space-y-2">
                    {orderItems.map((item) => (
                        <div key={item.itemId} className="flex justify-between items-center p-2 border-b border-gray-100 dark:border-gray-700">
                            <div className="flex items-center">
                                ...
                                />
                                <div>
                                    <p className="text-sm text-textDark dark:text-textLight truncate max-w-xs">{item.productName}</p>
                            ...
                            <span className="text-sm font-semibold text-primary">
                                {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format((item.unitPrice ?? 0) * item.quantity)}
                            </span>
                        </div>
                    ))}
                </div>
