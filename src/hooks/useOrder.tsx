...
const useOrder = () => {
    ...
    const requestLockRef = useRef(new Map<number, { inProgress: boolean; timestamp: number }>());

    // Get order by ID
    const getOrderById = useCallback(
        async (orderId: number): Promise<OrderResponseDTO> => {
            ...
            // Nếu currentOrder đã có và khớp với orderId, trả về ngay lập tức
            if (currentOrder && currentOrder.orderId === orderId) {
                return currentOrder;
            }

            // Kiểm tra khóa yêu cầu cho orderId cụ thể
            const lock = requestLockRef.current.get(orderId) || { inProgress: false, timestamp: 0 };
            const currentTime = Date.now();

            if (lock.inProgress) {
                throw new Error('Yêu cầu đang được xử lý hoặc gửi quá thường xuyên');
            }

            // Đặt khóa cho yêu cầu mới
            requestLockRef.current.set(orderId, { inProgress: true, timestamp: currentTime });
            ...
            try {
                ...
                requestLockRef.current.set(orderId, { inProgress: false, timestamp: currentTime });
                return orderData;
            } catch (error) {
                requestLockRef.current.set(orderId, { inProgress: false, timestamp: currentTime });
                const errorMessage =
    ...
    const getUserOrders = useCallback(
        async (
            ...
            try {
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể lấy danh sách đơn hàng';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const createOrder = useCallback(
        async (orderData: CreateOrderDTO): Promise<OrderResponseDTO> => {
            if (!isAuthenticated || !user?.userId) {
    ...
    const cancelOrder = useCallback(
        async (orderId: number, cancelData: CancelOrderDTO): Promise<OrderResponseDTO> => {
            if (!isAuthenticated || !user?.userId) {
    ...
    const getShippingMethods = useCallback(async (): Promise<ShippingMethodDTO[]> => {
        ...
        try {
            ...
            const errorMessage =
                error instanceof Error ? error.message : 'Không thể lấy phương thức vận chuyển';
            dispatch(fetchOrderFailure(errorMessage));
            throw error;
        }
    }, [dispatch]);
    ...
    // Estimate shipping
    const estimateOrderShipping = useCallback(
        async (
            orderTotal: number,
            ...
            dispatch(fetchOrderStart());
            try {
                const estimate = await OrderService.estimateOrderShipping(
                    orderTotal,
                    shippingMethodId,
                    totalWeight
                );
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể ước tính phí vận chuyển';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const createPayment = useCallback(
        async (paymentRequest: PaymentRequestDTO): Promise<PaymentResponseDTO> => {
            if (!isAuthenticated || !user?.userId) {
    ...
    const checkPaymentStatus = useCallback(
        async (transactionId: string): Promise<ApiResponse> => {
            if (!isAuthenticated || !user?.userId) {
            ...
            try {
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể kiểm tra trạng thái thanh toán';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const cancelPayment = useCallback(
        async (paymentId: number): Promise<ApiResponse> => {
            if (!isAuthenticated || !user?.userId) {
    ...
    const getOrderItems = useCallback(
        async (orderId: number): Promise<OrderItemDTO[]> => {
            if (!isAuthenticated || !user?.userId) {
            ...
            try {
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể lấy chi tiết đơn hàng';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const getPaymentByOrderId = useCallback(
        async (orderId: number): Promise<PaymentResponseDTO> => {
            if (!isAuthenticated || !user?.userId) {
            ...
            try {
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể lấy thông tin thanh toán';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const getPaymentMethods = useCallback(async (): Promise<PaymentMethodDTO[]> => {
        try {
            ...
            const errorMessage =
                error instanceof Error ? error.message : 'Không thể lấy phương thức thanh toán';
            dispatch(fetchOrderFailure(errorMessage));
            throw error;
        }
    }, [dispatch]);
    ...
    const verifyPaymentCallback = useCallback(
        async (queryParams: URLSearchParams): Promise<PaymentResponseDTO> => {
            dispatch(fetchOrderStart());
            try {
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể xác minh thanh toán';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const pollPaymentStatus = useCallback(
        async (
            ...
            try {
                const userId = user.userId;
                const paymentStatus = await OrderService.pollPaymentStatus(
                    userId,
                    orderId,
                    intervalMs,
                    timeoutMs
                ...
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể theo dõi trạng thái thanh toán';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const getBestsellingProducts = useCallback(
        async (limit: number = 10): Promise<BestsellingProductDTO[]> => {
            try {
                return await OrderService.getBestsellingProducts(limit);
            } catch (error) {
                const errorMessage =
                    error instanceof Error ? error.message : 'Không thể lấy danh sách sản phẩm bán chạy';
                dispatch(fetchOrderFailure(errorMessage));
                throw error;
            }
        },
    ...
    const getOrderByIdFromState = useCallback(
        (orderId: number): OrderSummaryDTO | null => {
    ...
    const getShippingMethodById = useCallback(
        (methodId: number): ShippingMethodDTO | null => {
    ...
    const getOrdersByStatus = useCallback(
        (status: OrderStatus): OrderSummaryDTO[] => {
